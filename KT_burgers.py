...
def phi(theta):
  ...
  # MC
  # return minmod3( 2., 2.* theta, 0.5*(1+theta) )
  # vanLeer
...
def calc_RHS(U):
  ...
  # flux limiter
  theta = dU_L / (dU_R + 1.e-10)
  sigma = phi(theta) * dU_R
...
def step():
    ...
    RHS_1 = calc_RHS(Q)
    Q_1[2:-2] = Q[2:-2] + dt * calc_RHS(Q)
    boundary(Q_1)
    
    RHS_2 = calc_RHS(Q_1)
    Q[2:-2] += dt * 0.5 * ( RHS_1 + RHS_2 )
    boundary(Q)
